# LA FIRME — PEA Screener Pro v3.0

_Derniere mise a jour : 2026-03-08 07:51 UTC_

## Statistiques
- **Actions analysees** : 132
- **Candidats (>=55)** : 15
- **Score moyen** : 43.8/100

## Top 10 Candidats
| # | Ticker | Nom | Secteur | Score | Q | V | M | S | Verdict |
|---|--------|-----|---------|-------|---|---|---|---|---------|
| 1 | **GTT.PA** | Gaztransport | Energie | **71.0** | 87.2 | 31.6 | 92.5 | 70.4 | CANDIDAT (>=55) |
| 2 | **INVE-B.ST** | Investor AB | Finance | **68.1** | 61.7 | 75.2 | 65.5 | 74.0 | CANDIDAT (>=55) |
| 3 | **SAMPO.HE** | Sampo | Finance | **66.2** | 77.2 | 65.4 | 52.0 | 62.2 | CANDIDAT (>=55) |
| 4 | **ERIC-B.ST** | Ericsson | Tech | **63.7** | 65.9 | 53.2 | 79.6 | 52.8 | CANDIDAT (>=55) |
| 5 | **NOS.LS** | NOS SGPS | Telecom | **61.8** | 69.5 | 60.5 | 76.2 | 30.7 | CANDIDAT (>=55) |
| 6 | **FGR.PA** | Eiffage | Industrie | **61.4** | 55.2 | 78.5 | 77.1 | 23.9 | CANDIDAT (>=55) |
| 7 | **ASML.AS** | ASML Holding | Tech | **60.6** | 76.8 | 15.6 | 88.6 | 58.4 | CANDIDAT (>=55) |
| 8 | **PRY.MI** | Prysmian | Industrie | **59.0** | 63.7 | 31.2 | 94.9 | 43.5 | CANDIDAT (>=55) |
| 9 | **ITX.MC** | Industria de Diseno | Conso | **58.6** | 75.7 | 28.3 | 66.0 | 57.1 | CANDIDAT (>=55) |
| 10 | **GALP.LS** | Galp Energia | Energie | **57.3** | 45.8 | 58.3 | 77.0 | 51.8 | CANDIDAT (>=55) |

## Signaux Forward-Looking (Top Candidats)
| Ticker | Révision Proxy | Dist 52S | Accruals | Liquidité | Value Trap |
|--------|---------------|----------|----------|-----------|------------|
| GTT.PA | — | -2.3% |  | OK | non |
| INVE-B.ST | ATTENTION - REVISION NEGATIVE POSSIBLE | -4.0% |  | OK | non |
| SAMPO.HE | ATTENTION - REVISION NEGATIVE POSSIBLE | -11.5% |  | OK | non |
| ERIC-B.ST | ATTENTION - REVISION NEGATIVE POSSIBLE | -1.2% |  | OK | non |
| NOS.LS | ATTENTION - REVISION NEGATIVE POSSIBLE | 0.0% |  | ATTENTION | non |
| FGR.PA | FORTE HAUSSE ATTENDUE | -6.0% |  | OK | non |
| ASML.AS | FORTE HAUSSE ATTENDUE | -7.9% |  | OK | non |
| PRY.MI | FORTE HAUSSE ATTENDUE | -7.1% |  | OK | non |
| ITX.MC | REVISION POSITIVE | -8.5% |  | OK | non |
| GALP.LS | — | -2.0% |  | OK | non |

## Architecture Scoring
| Pilier | Poids | Criteres principaux |
|--------|-------|---------------------|
| Qualite | 35% | ROIC proxy, ROE, Marge brute, Croissance CA, Piotroski |
| Valeur | 25% | EV/EBITDA, P/FCF, P/Book, FCF Yield |
| Momentum | 20% | 12-1M, 6M, EPS Growth |
| Solidite | 15% | Altman Z, Dette/EBITDA, Current Ratio |
| Signaux Fwd | 5% | Accruals, Distance 52S |

> _Ce screener ne constitue pas un conseil en investissement._
> _Toujours valider avec une analyse qualitative approfondie._